...
### B42 — REALTIME: недокоммиченный ответ диспетчера ТЕРЯЕТСЯ при «Завершить — в чат» посреди речи — CRIT — fixed (`6705412`)
...
### B43 — REALTIME: гонка реконнекта переклеивает живой звонок на ДРУГОЙ тред → история расщепляется — MAJOR — fixed: сервер (`7c66617`) + клиентский гейт render `!client && !connecting && !liveRequested`
...
### B44 — REALTIME: реконнект обнуляет живой LLM-контекст диспетчера — лента полная, но диспетчер «забыл» всё до обрыва — MAJOR — fixed (`7bc3abe`)
...
### B45 — тап по заголовку треда (rename) сносит `#view-title` из DOM → следующий render кидает → ВСЕ обновления title/badge/стадии замирают до перезагрузки — CRIT — fixed (live-верифицирован Playwright): restore-узел первым в commitRename + setViewTitle null-гвард для фоновых писателей (render/loadLists кидали TypeError каждые 5с, пока редактор открыт)
...
### B46 — write_code stale-plan гвард переоткрыт: завершение НЕсвязанной direct-dispatch задачи ставит `last_outcome="completed"` → Кора кодит по НЕвалидированному плану (реопен B07) — CRIT — fixed (`0e33153`)
...
### B47 — direct-dispatch (`submit_task`/`confirm_task`) НЕ двигает `thread.stage` → завершённая задача навсегда с бейджем «СБОР» (наблюдалось живьём) — MAJOR — fixed (`0e33153`)
...
### B48 — архивный тред остаётся полностью запускаемым/мутируемым: `archived` не проверяется ни в одном write-пути — MAJOR — fixed (`3999cde`)
...
### B49 — архив треда разрешён, пока задача в `PENDING_CONFIRMATION` (busy-чек ловит только RUNNING) — MAJOR — fixed (`3999cde`)
...
### B50 — `/api/browse?path=%00` (null-байт) → 500 unhandled на неаутентифицированном роуте — MAJOR — fixed (`268e04c`)
...
### B51 — `confirm`/`fast` строка-vs-bool: `bool("false")==True` → `confirm:"false"` запускает НЕподтверждённый full-write Kora-ран — MAJOR — fixed (`268e04c`)
...
### B52 — gate-карточка навсегда disabled после 409 «busy», нет пути ретрая — MAJOR — fixed (live-верифицирован Playwright): 409 → note «попробуй ещё раз» + кнопки ре-энейблены
...
### B53 — успешный gate-экшен оставляет note «запускаю…» и ре-энейблит те же кнопки → приглашение к дублю; `live` захвачен один раз, устаревает — MAJOR — fixed (live-верифицирован Playwright): consumed-флаг — успех держит кнопки disabled + note «готово ✓», повторный клик не шлёт /gate
...
### B54 — архив треда, который открыт → страница молча деградирует в пустую, ноль фидбека — MAJOR — fixed (live-верифицирован Playwright): навигация на дом + setConn «тред … в архиве»; render показывает «тред не найден» при listsLoaded
...
### B55 — неизвестный/стейл тред-роут → пустая вью + бесконечный 404-поллинг ленты, нет «не найдено» — MINOR — fixed (live-верифицирован Playwright: 1 запрос вместо вечного цикла): err.status на getJSON, feedNotFound глушит поллинг + event-строка «тред не найден или удалён», сброс при смене треда
...
### B56 — `GET /api/threads?archived=false` трактуется truthy → прячет реальный список тредов — MINOR — fixed (`268e04c`)
...
### B57 — `feed?limit=0` возвращает ВСЮ ленту (и `limit=-5` — левый срез) — MINOR — fixed (`268e04c`)
...
### B58 — `POST /api/active-thread {"id":""}` → 404 вместо очистки активного треда — MINOR — fixed (`268e04c`)
