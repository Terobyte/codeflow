...
> Эта спека остаётся **нормативной** для открытых слайсов KV-*/AI-*/R-* и их
> гейтов/проб (§0, §14). Единая карта проекта —
> [`docs/synapse-master-spec.md`](../../synapse-master-spec.md).
...
| Probe P3b Pipecat voice switch/correlation | **ЗАКРЫТ 2026-07-15 → NO-GO для KV-1** (`78d17da` корреляция + `f192a3d` latency, артефакт `probes/ARTIFACTS.md`) | артефакт §14.2.4, p95 ≤ 500 мс, exact `context_id ↔ utterance_id`, без потерь | KV-1 |
...
**P3b сработал как развилка, а не как формальность (2026-07-15).** Корреляция
дала GO (`78d17da`, живое подтверждение в `f192a3d`: 40/40 реплик, реальный WS).
Latency дала NO-GO и не пограничный: свитч голоса — это полный реконнект WS
(`fish/tts.py:219-236`), и он стоит **+707…+754 мс парной дельты** (switched p95
877–1200 мс) при пороге 500. Проверено на обеих моделях, включая платную
`s2-pro`, — дело не в промо-тарифе. Механика подтверждена счётчиком: 21
`on_connected` на 20 свитчей, baseline не реконнектит. Следствие: **voice-aware
арбитр §4.1 как единственный механизм голоса Коры не строится**; по правилу выше
это отдельная transport-спека. KV-1 закрыт как no-go, и вместе с ним подвисает
KV-3a (§0.2: KV-3a ← KV-1), т.е. весь контур A кроме KV-2/AI-ветки. Решение о
замене — за владельцем спеки, не за исполнителем слайса.

**Решение владельца (2026-07-15): KV-1 разрезан, построена уцелевшая половина.**
KV-1 склеивал две несвязанные вещи, и no-go убил только одну. Play-путь идёт
через REST (`fish_rest_tts`), а не через WS: голос там — параметр, реконнекта
нет, цена свитча ноль. Отсюда **KV-1a — «Play-кнопка перестаёт врать»**
(`d34a5b4`): роль `disp|kora` → reference id, voice-aware кэш, неизвестная роль
→ 400. Гейта не требует, §1-жалобу №1 («подпись врёт») закрывает. Живая половина
(голос Коры в тракте) ждёт отдельной transport-спеки; KV-3a остаётся подвешенным
до решения, идёт ли присутствие Коры на одном голосе.

Две находки probe-а, которые эта спека обязана переварить отдельно:

1. **Порог §14.2.4 сформулирован так, что меряет не тот объект.** Буквальное
   чтение — «switched p95 ≤ 500 мс» — включает собственный TTFB синтеза Fish,
   который свитч не создаёт и KV-1 не устранил бы. На живых прогонах *baseline*
   p95 (вообще без свитча) сам заходил за 500 мс (458–722), т.е. буквальный порог
   местами недостижим на Fish в принципе. Нормативным должно быть чтение «цена
   свитча» = парная дельта. На сегодняшний исход это не влияет (оба чтения
   NO-GO), но §0.1 обязан зафиксировать чтение до следующего порога.
2. **Требование «exact correlation при SPEAK reordering/drop» неисполнимо как
   probe к платформе.** Протокол Fish не эхоит `context_id`: сервис проставляет
   его из локального курсора (`tts_service.py:1371-1384`, где это прямо названо
   fallback-ом «для сервисов, чей протокол не эхоит context_id»). На проводе нет
   id, который можно переставить, — это вопрос к сериализации арбитра Синапса, а
   не к Pipecat/Fish. Требование нужно переписать на арбитра или снять.
