...
Статус: **v3 — после аудита против кода, готов к ревью Теро**. v2 порезан: аудит
...
| --- | --- |
| Явная зависимость от Фазы 0 (С4/С5) | С4 (`GuardedLLMClient` — общий cost cap, нормализация сбоев) уже в рабочем дереве; С5 (bearer authn) уже соспецован. Старые AI-0/AI-2 описывали ту же работу как новую |
...
- `max turns`, бюджет запуска (USD), deadline (сек);
...
| --- | --- | --- |
| Голосовой диспетчер | pipecat-каскад, `build_session_pipeline` (`app.py:888`) | **да** — snapshot на сборку сеанса |
...
Только `.env` через `SynapseConfig.from_env()` — как сегодня. `GET
/api/settings/ai` отдаёт по провайдеру: `configured: bool`, `source: "env" |
"none"`, необратимую маску (`sk-…7f2a`). Ничего мутирующего. Смена ключа =
правка `.env` + рестарт — честно и названо в UI подсказкой.

### 6.3. Порядок обёрток текстового канала

Фаза 0 С4 уже создала шов: `роут → GuardedLLMClient → AnthropicLLMClient`
(`synapse/dispatcher/llm_client.py:110`). Маршрутизация встраивается так:
...
новых типов исключений роут не учит, обработка С4 работает без правок.
...
- `ProviderUnavailable` от primary → одна попытка fallback → если и он упал,
  роут отдаёт существующую degraded-реплику (200 + `degraded: true`, С4);
- полиморфизм `LLMClient.complete()` сохраняется — loop и консоль разницы не видят.

### 6.4. Голосовой канал

`build_session_pipeline` (`app.py:888`) уже пересобирает pipecat-сервисы на
...
1. `SynapseConfig` остаётся immutable bootstrap (пути, порты, стартовая
   миграция env). Runtime-код настроек читает только snapshot-ы store-а.
2. **Миграция первого старта**: нет `ai-settings.json` → создать из текущего
   env-состояния: primary = OpenRouter/`tier1_model`, fallback =
   Anthropic/`tier2_model`, kora = `kora_model`/`kora_max_turns`/
   `kora_max_budget_usd`/`kora_deadline_s`. `.env` не изменяется → rollback на
   старую версию безопасен.
3. `build_session_pipeline` (`app.py:888`): сервисы из Registry по snapshot-у
...
2. Выбранная модель исчезла из списка провайдера → карточка показывает
   `модель недоступна`; ход идёт в fallback или отдаёт настроечную ошибку.
...
9. Primary и fallback оба недоступны → одна degraded-реплика (существующий
   путь С4), не каскад 500.
10. Провайдер вернул текст без требуемого tool-call контракта → contract
    error, НЕ сетевой fallback.
11. Рестарт сервера → настройки восстановлены из файла; отсутствие файла на
    первом старте → миграция из env (§7.2).

## 11. Скоуп реализации

| Слайс | Содержание | Зависимости |
| --- | --- | --- |
| **AI-0 — пререквизит, не работа** | Фаза 0 С5 (authn) и С4 (cost cap + fallback) влиты | Фаза 0 |
...
| Gemini text-адаптер: tool-shape отличается от Anthropic/OpenRouter | Специфика не выходит за адаптер; contract error ≠ сетевой fallback (§10.10) |
...
6. Голосовой и текстовый ход при одном snapshot-е выбирают один маршрут
   (parity-тест).
7. Ход всегда начинается с primary; timeout/429/5xx переключают на fallback;
   400/contract/security — нет; `CostCapBlocked` не уходит в платный резерв.
8. Один провайдер вызывается не более раза на LLM-pass.
9. Изменение настроек не меняет активный ход, активный голосовой сеанс и
   активный `RunSpec`.
10. Gate-карточка переопределяет модель Коры на один запуск; невалидная модель
    отбрасывается сервером.
11. Конкурентное сохранение → 409, без потери данных; рестарт восстанавливает
    настройки; первый старт без файла мигрирует из env.
12. Breaker, затриповавший маршрут в голосе, виден тексту (общий ключ).
13. Полная тестовая суита остаётся зелёной; замороженные тесты не тронуты.
...
- `synapse/config.py` — bootstrap env; источник миграции §7.2.
