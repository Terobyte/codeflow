...
class DispatcherTurnLoop:
    ...
    def clear_history(self, thread_id: str) -> None:
        ...
        B-DISP-7: кэш треда при этом ВЫКИДЫВАЕТСЯ, а не остаётся пустым. Тёплая история
        обязана совпадать с тем, что вернула бы холодная регидрация; держа пустой список в
        кэше, мы оставляли `note_external_turn` (войс-путь, ВНЕ turn_lock) кэш-хит, и он
        дописывал туда реплику, которую `history_from_feed` уже отрезала своим срезом по
        последнему clear-маркеру — каналы расходились. После выкидывания тот же вызов уходит
        в штатный no-op по кэш-миссу, а решает ЛЕНТА: легла реплика до маркера — холодная
        регидрация её отрежет, легла после — вернёт. Один источник правды вместо двух.
        Инвалидация внутри контракта: `_histories` и так LRU-вытесняет списки.
        NB: маркер обязан лечь в ленту ДО этого вызова (роут держит оба под turn_lock) —
        иначе чтение в окне «кэша уже нет, маркера ещё нет» регидрирует НЕочищенную историю
        из ленты и отменит clear."""
        with self._history_lock_for(thread_id):
            with self._history_index_lock:
                hist = self._histories.pop(thread_id, None)
            if hist is not None:
                hist[:] = []
