...
class KoraRunner:
    ...
    def _system_prompt(self, workspace: Path, task_text: str) -> str:
        ...
        return (
            ...
            f"shell. "
            # KV-2 §4.2 (речевой контракт): текст абзаца задан спекой дословно. Он про ТЕКСТ
            # для пользователя, не про инструменты — стоит после гейт-правил и до задачи,
            # чтобы «не пиши пути» не читалось как «не трогай файлы» (LOAD-BEARING выше:
            # промпт обязан называть workspace, иначе Кора выдумывает пути). Детерминированный
            # speakable() — только страховка формата, промпт — первичный рычаг.
            f"Твои текстовые сообщения пользователь слышит голосом. Пиши их разговорно, "
