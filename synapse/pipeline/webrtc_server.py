...
`build_pipeline()` stays transport-agnostic; we only wrap it per browser session. aiortc/cv2/
fastapi/pipecat_ai_prebuilt are imported here (behind the `voice` extra), never at app.py top (S4).
"""
...
def build_web_app(cfg: SynapseConfig) -> FastAPI:
    """FastAPI app: POST/PATCH /api/offer drive pipecat's SmallWebRTC signaling, the prebuilt
    browser client is mounted at /client. Each new browser offer spins one fresh voice pipeline
    session (`run_session`). cfg is captured by the closure and only touched per-connection, so
    build_web_app itself needs no keys/network (lets the unit test call it with a dummy cfg)."""
    app = FastAPI()
    ...
    async def run_session(connection: SmallWebRTCConnection) -> None:
        voice_pipeline = build_pipeline(cfg)
        transport = SmallWebRTCTransport(
            webrtc_connection=connection,
            params=TransportParams(audio_in_enabled=True, audio_out_enabled=True),
        )
        full = Pipeline([transport.input(), voice_pipeline.pipeline, transport.output()])
        ...
        async def _on_client_disconnected(_transport, _client):
            # M1: browser close/refresh pushes no EndFrame -> cancel so Flux/Fish/LLM sockets +
            # monitor_forever() tear down instead of leaking until the process exits.
    ...
    async def _handle_offer(request: SmallWebRTCRequest, background_tasks: BackgroundTasks):
        async def on_connection(connection: SmallWebRTCConnection) -> None:
            background_tasks.add_task(run_session, connection)
